<script>
  import { onMount } from 'svelte';
  import { Lucide } from '@lucide/astro';

</script>

<div class="navbar shadow-sm bg-gray-900">
  <div class="navbar-start">
    <div class="dropdown ">
      <div tabindex="0" role="button" class="btn btn-ghost lg:hidden text-white hover:bg-transparent">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" /> </svg>
      </div>
      <ul
        class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow text-black ">
        <li><a href='/'>About</a></li>
        <li><a href='/'>Skills</a></li>
        <li><a href='/'>Experience</a></li>
        <li><a href='/'>Certifications</a></li>
        <li><a href='/'>Contact</a></li>

      </ul>
    </div>
    <a href='/' class="text-xl text-white px-5">BM</a>
  </div>
  <div class="navbar-center hidden lg:flex">
    <ul class="menu menu-horizontal px-1 text-white">
      <li><a href='/'>About</a></li>
        <li><a href='/'>Skills</a></li>
        <li><a href='/'>Experience</a></li>
        <li><a href='/'>Certifications</a></li>
        <li><a href='/'>Contact</a></li>
    </ul>
  </div>
  <div class="navbar-end px-5">
    <a href="./assets/CV.pdf" download ><button class="btn bg-red-500 rounded-4xl text-white border-none">Hire Me</button></a>
  </div>
</div>

